...
(function (root, factory) {
   ...
   * файл. Ноль — «взять встроенный размер»; `1..100` — доля экрана; `≥101` —
   * пиксели без верхней границы (её, как и раньше долевую, ставит только
   * экран — `wanted_size`/`fit_to_screen` в Rust). Дробное меньше единицы не
   * значит ничего ни как доля, ни как пиксели, и тоже уходит в ноль. Две
   * проверки тут неизбежны (размер ставит Rust, показывает выбор страница),
   * поэтому они и записаны одинаково; сторож сверяет границы Rust на общих
   * значениях (`test/config-shape.test.js`, «границы доли те же»).
   */
  function normalizePickerSize(raw) {
    ...
    const axis = (half, side) => {
      ...
      if (value === 0) return 0;
      if (value >= 1) return value; // 1..100 — доля экрана, ≥101 — пиксели
      return 0;
    };
    return {
