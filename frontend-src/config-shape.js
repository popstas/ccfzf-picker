...
(function (root, factory) {
  ...
  const DEFAULTS = {
    ...
    sshHost: '',
    // Второй источник списка: `ccfzf` на этой машине, без ssh. Включён по
    // умолчанию: свои сессии человек ждёт в списке, ничего не настраивая, а
    // забытая галка выглядит не настройкой, а пропажей сессий — так и вышло на
    // макбуке, где работающая местная сессия не показывалась вовсе, притом что
    // на соседнем маке тот же пикер её показывал.
    //
    // Цена известна: на нативном Windows местного источника нет вовсе —
    // `ccfzf` это bash-обёртка вокруг встроенной python-программы, а `bash.exe`
    // там ведёт в WSL, к чужому домашнему каталогу. Такой машине ключ пишется в
    // конфиг явным `false`; забытый оборачивается строкой об отказе источника в
    // статуслайне — видной, а не молчащей.
    localSource: true,
    hotkey: 'Cmd+Shift+C',
    ...
    //
    // Одно слово, а не argv: как именно звать редактор, решает Rust
    // (`editor_argv` в main.rs), потому что это зависит от системы. На маке
    // голое имя уходит в `open -a`: CLI редактора там может не быть вовсе —
    // проверено с cursor, `which cursor` пуст, а Cursor.app на месте. Полный
    // путь в этом же поле значит «зови напрямую».
    //
    // Кому нужна форма посложнее — заводит своё действие в `actions`, там argv
    // свой.
    editor: 'cursor',
