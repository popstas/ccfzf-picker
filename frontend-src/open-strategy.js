(function (root, factory) {
   ...
   *
   * Пикер её не запускает и запустить не может: `reptyr -T` лезет к держателю
   * мастера pty, а это `sshd`, сбросивший привилегии через setuid, — ptrace по
   * нему запрещён своему же юзеру (проверено 2026-08-05, см.
   * docs/reptyr-experiment.md). Под root запрет снимается, а пароль к sudo
   * вводит тот, кто сидит за терминалом. Отсюда и форма: не действие, а текст
   * для вставки.
   *
