...
(function (root, factory) {
   ...
   *
   * Нулевой `windowPid` тоже запрещает фокус. Windows отдаёт передний план
   * только владельцу либо тому, кто получил последнее событие ввода; трекеру
   * это право выдаёт пикер по pid, и без pid подъём отчитался бы об успехе, а
   * на экране мигнула бы кнопка на таскбаре. Молча не сработавший Enter хуже
   * прежнего поведения.
   */
  ...
  /** pid демона трекера; ноль значит «права выдавать некому». */
