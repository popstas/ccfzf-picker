...
(function (root, factory) {
   ...
   * `▣` — открытое окно. `●` — не тот же знак, что в списке (там
   * непросмотренная сессия горит кружком `.dot.review`, крашенным CSS), а
   * тот же цвет и та же круглая форма: совпадение достаточное, чтобы знак
   * читался как то же самое дело.
   */
  const GLYPHS = {
    new: { text: '+' },
    info: { text: 'ⓘ' },
