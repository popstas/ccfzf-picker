...
(function (root, factory) {
   ...
   * секцию можно перенести и в соседнюю колонку, поэтому порядок там —
   * последовательность на каждую колонку: так и колонка, и место внутри
   ...
   * секций этот разбор не может вовсе: `past:<N>` приходят и уходят вместе с
   * рабочими столами, и выброси мы ключ опустевшего, его место забылось бы за
   * один опрос. Незнакомое отсеивает уже `buildSections`, где виден настоящий
   * набор.
   */
