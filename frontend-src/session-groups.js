...
(function (root, factory) {
   ...
   *
   * `opts.hideDesktop` убирает сессии, открытые приложением Claude Desktop.
   * Имя отрицательное, в отличие от галки `showDesktop`, которой оно
   * управляется: в этой тройке все поля значат «отсев включён», и
   * положительное пришлось бы разворачивать в `activeFilters` второй формой.
   *
   ...
   *
   * Непустой запрос и любой префикс режима отменяют все отсевы — `onlyLive`,
   * `onlyWindow` и `hideDesktop`. Причина та же, по которой непустой запрос разворачивает
   ...
   *
   * Болеют этим все одинаково, поэтому и правило одно. `onlyWindow` вычищает
  ...
  function activeFilters(opts = {}) {
    ...
    return {
      ...
      hideDesktop: asked ? false : !!opts.hideDesktop,
    };
  }
  ...
  function buildSessionsPayload(res, sort = DEFAULT_SORT, opts = {}) {
    ...
    // Раньше onlyWindow: тот считает приложение окном (см. ниже), и стой
    // отсевы в обратном порядке, строка приложения пережила бы тот самый
    // отсев, которым её и просили убрать. Предикат общий с глифом и с
    // onlyWindow: свой здесь разошёлся бы с нарисованным молча.
    if (opts.hideDesktop) rows = rows.filter(r => !listApi.inDesktopApp(r));
    if (opts.onlyWindow) {
