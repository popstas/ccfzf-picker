...
(function (root, factory) {
   ...
   * (`agentUpdated === 0` — нечего отматывать), и у строки, которую и так не
   * читали: на маке «читал» — это `agentSeen`, отметку ставит открытие сессии.
   *
   * Спрашивается именно `agentUpdated`, а не `lastActivity`: у второго
   * появился откат на время транскрипта, и он ненулевой теперь у всякой
   * сессии — в том числе у той, про которую хук не писал ни разу. Оба пункта
   * там были бы обещанием: отметка легла бы в `seen.json`, а показать её
   * нечем — и кружок, и текст статуса завязаны на пустой `agentState`.
   *
   ...
   *
   * `opts.os` — система самого пикера, той же меркой, что у
   * `buildOpenCommand`. Необязательна по той же причине: не назвали — правило
   * про Windows выключено, и список прежний. Читает её единственная проверка
   * `isWindowsLocalRow` (open-transport.js), и все пять мест ниже спрашивают
   * именно её, а не пишут по второму условию: факт один, последствий у него
   * два — нет шелла и некому писать комментарий, — и разъехались бы они молча.
   */
  function availableActions(row, config, opts) {
    const actions = [];
    const cfg = config || {};
    // Своя машина на Windows: местная команда пошла бы через `/bin/sh -c`,
    // которого там нет, а `--comment` некому разобрать. Причина у каждого
    // места своя и написана на месте; мерка — одна.
    const winLocal = transportApi.isWindowsLocalRow(row, (opts || {}).os);
    ...
    if ((row || {}).kind === 'project') {
      // Оба пункта кончаются шеллом: `New session` откатывается в местную
      // `newSession`, а `Open terminal` уходит в неё всегда — менеджера он не
      // просит вовсе. На Windows у своей строки это `/bin/sh -c`, то есть
      // чужая ошибка разбора вместо окна. Enter у такой строки тоже меняет
      // дело — см. `openProjectRow` в sessions.html.
      const forProject = winLocal ? [] : [
    ...
    if ((row || {}).kind === 'snapshot-session') {
      ...
      // Та же мерка, что у строки проекта и у общей ветки: копий у этих двух
      // пунктов три, и правило обязано стоять при каждой — иначе оно
      // работало бы через строку.
      if (row.cwd && !winLocal) {
    ...
    if (row && row.agentUpdated && row.agentSeen) actions.push({ id: 'unread', label: 'Mark unread' });
    ...
    if (row && row.agentUpdated && !row.agentSeen) actions.push({ id: 'seen', label: 'Mark seen' });
    ...
    //
    // Сессии приложения пункт не достаётся ни на одной системе, и это не
    // осторожность: reptyr перевешивает процесс на **другой** терминал, а у
    // сессии Claude Desktop терминала нет вовсе — перехватывать нечего. pid у
    // неё при этом настоящий (агрегатор находит процесс), то есть без этой
    // оговорки пункт стоял бы в меню и давал команду, которой не за что
    // взяться.
    //
    // На Windows у своей строки его нет по второй причине: команда уходит
    // через `/bin/sh -c`, а `reptyr` — про POSIX-машину.
    if (row && row.live && !transportApi.isDesktopRow(row) && !winLocal
      && openApi.buildAttachCommand(row)) {
    ...
    //
    // На Windows пункта нет: у **живой** сессии `chooseOpenStrategy` даёт не
    // `resume`, а `takeover`/`attach`, и windows-ветка `buildOpenCommand`,
    // которая одна и умеет обойтись без шелла, их не покрывает — команда
    // уходит через `/bin/sh -c`, и терминал отвечает разбором своей командной
    // строки. Дорога у такой строки остаётся одна, Enter в приложение.
    if (transportApi.isDesktopRow(row) && !winLocal) {
    ...
    //
    // На Windows у своей строки пункта нет всегда — в том числе там, где
    // просьбу принял бы менеджер: `chooseProjectOpenAction` решает транспорт
    // уже после того, как пункт нарисован, а меню не может обещать через
    // строку. Решение владельца: под Windows новую сессию заводят не отсюда.
    if (row && row.cwd && !winLocal) actions.push({ id: 'new', label: 'New session' });
    ...
    // Менеджера этот пункт не просит вовсе, то есть на Windows у своей строки
    // он кончается `/bin/sh -c` без всяких развилок.
    if (row && row.cwd && !winLocal) actions.push({ id: 'terminal', label: 'Open terminal' });
    ...
    //
    // На Windows у своей строки писать его некому: местный агрегатор зовётся
    // там как `python ccfzf.py`, а ветку `--comment` разбирает bash-обёртка,
    // которой на этой машине нет вовсе. `mode` становится `"--comment"`, ни
    // одна ветка блока не совпадает, текст со stdin пропадает, и блок выходит
    // с нулём — то есть пикер отказа не видит и рисует комментарий
    // сохранённым. Через ssh пункт по-прежнему работает, поэтому мерка по
    // строке, а не по системе.
    if (row && row.id && !winLocal) actions.push({ id: 'comment', label: 'Edit comment' });
