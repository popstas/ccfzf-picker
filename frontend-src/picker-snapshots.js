    ...
    for (const snap of sorted) {
      ...
      const rows = all
        .map(s => ({
          ...
          // Имя сессии — второй подписью рядом с именем проекта
          // (snapshotNameHtml): сессий одного каталога в снимке бывает
          // несколько, и различить их было нечем. Пустая строка, а не
          // отсутствие поля: из полей склеивается стог поиска, и недостающее
          // уехало бы в него словом `undefined` — запрос из него нашёл бы
          // разом весь снимок. То же правило, что у `windowTitle` в
          // session-list.js.
        ...
        // Имя сессии в стогу вторым — по той же причине, по какой там стоит
        // имя окна у строки сессии: показанное в строке обязано и находиться,
        // иначе запрос по видимому глазами имени даёт ноль строк.
        .filter(r => filterApi.matchesText(
          `${r.label} ${r.title} ${filterApi.searchableCwd(r.cwd)}`, q));
