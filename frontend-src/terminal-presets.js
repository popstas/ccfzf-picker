...
(function (root, factory) {
   ...
   *
   * Строку эту отдают `write text`, а не параметру `command` у `create
   * window`, и разница не косметическая: `command` разбирает **сам iTerm2**
   * своим токенизатором, а тот обрабатывает `\` и внутри одинарных кавычек —
   * там, где POSIX-шелл считает его обычным знаком. Идиома `'\''`, которой
   * `q` закрывает кавычку, от этого рассыпается, и до ssh доезжает битая
   * команда: она падает мгновенно, а профиль по умолчанию закрывает сессию,
   * когда та кончилась, — то есть окно открывается и тут же исчезает, без
   * единого слова о причине (ответа `osascript` мы не читаем). Измерено на
   * живом маке 2026-08-15, разбор — в тесте про шелл сессии.
   *
   * `write text` печатает строку в уже поднятую сессию, и разбирает её шелл
   * этой сессии — то есть все три слоя кавычек (POSIX внутри POSIX внутри
   * AppleScript) попадают туда, где их понимают. Побочно окно остаётся
   * открытым после команды — как у kitty с `--hold`, и по той же причине:
   * иначе не прочитать, чем кончилась сессия.
   *
  ...
  const PRESETS = [
    ...
    },
    {
      ...
      args: ['-e', 'tell application "iTerm" to tell (create window with default profile) to tell current session to write text "{command}"'],
    },
    {
