...
pub fn press(app: &tauri::AppHandle, cwd: &str) {
    ...
    // Клон уходит в поток: сам `app` дальше этой функции не идёт, а
    // `spawn_blocking` требует `'static` — ссылку туда не занести.
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        } else if let Some(poller) = app_handle.try_state::<crate::poller::Poller>() {
            // Эта ветка — единственная дорога терминала мимо пяти команд
            // main.rs, и там всплеск опроса уже заводит каждая из них; здесь
            // его не заводил никто. Без него список узнавал о свежей сессии
            // хоткея через минуту-восемь бэкоффа — и второе нажатие того же
            // хоткея заводило вторую сессию с тем же именем, потому что
            // первая ещё не попала в снимок. Запасная дорога чуть выше
            // (событие `project-hotkey`) в этом не нуждается: она уходит на
            // страницу и там же зовёт `openProjectHere` — те самые команды
            // main.rs, где всплеск уже стоит.
            //
            // `try_state`, а не `state`: паника здесь обошлась бы дороже
            // пропущенного всплеска — та же причина, по которой чуть выше, у
            // `Registered`, взят тот же приём.
            poller.opened();
        }
    });
}
...
mod tests {
    ...
    /// `press` — шестая ветка, кончающаяся терминалом (первые пять — команды
    /// `main.rs`, у своего сторожа), и забытый здесь всплеск не падает и не
    /// молчит: нажатие по-прежнему открывает терминал, только список узнаёт о
    /// нём через минуту-восемь бэкоффа, а не через секунды. Увидеть это можно
    /// только глазами и только на Windows — ровно там, где менеджер этой
    /// машины и берётся открывать.
    #[test]
    fn press_starts_a_burst_on_the_manager_road() {
        let src = include_str!("project_hotkeys.rs");
        let (_, body) = src.split_once("fn press(").expect("press пропала — тест сторожит не то");
        let (body, _) = body.split_once("\n}\n").expect("функция не закрыта");
        assert!(
            body.contains("poller.opened()"),
            "press берёт менеджер этой машины открывать терминал и обязана завести всплеск опроса"
        );
    }
}
