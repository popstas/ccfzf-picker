...
//! `test/scrim.test.js`, «scrim не второй webview»: он грепает этот файл и
//! `main.rs` на строитель webview-окон Tauri и на метку `"scrim"` — так что
//! этому файлу нельзя даже упоминать в тексте название того строителя.
//!
//! Подложка **обязана** принимать клик, а не пропускать его насквозь: клик
//! мимо пикера должен снять с него фокус (`hideOnBlur`) точно так же, как
//! клик по голому столу. Пропусти подложка клик — он ушёл бы окну под ней, и
//! пикер остался бы висеть поверх чужой работы, ради ухода от чего его и
//! гасят по потере фокуса. Отсюда общее для обеих платформ правило: на
//! Windows это отказ от `WS_EX_TRANSPARENT`, на macOS —
...
mod win {
    ...
    ///
    /// `HWND` не `Send`, а хранить его надо между вызовами команд Tauri
    /// (которые могут прийти с разных потоков — см. правило про
    /// `run_main_thread!` в CLAUDE.md); значение поэтому лежит числом, тем же
    /// приёмом, каким на macOS-стороне этого же файла хранится указатель на
    /// `NSWindow`. Само окно создаётся и трогается только с главного потока
    /// — то же требование, что и у остальных Win32-вызовов в `main.rs`.
    ...
    fn ensure_window(app: &tauri::AppHandle) -> Result<HWND, String> {
        ...
        // переставит `set_visible` по свежему монитору пикера; здесь только
        // самая скромная заглушка, чтобы не заказывать окно нулевого размера
        // до того, как что-то попросит его показать.
        let hwnd = unsafe {
            CreateWindowExW(
...
mod mac {
    ...
    /// нам нужно держать его между вызовами команд Tauri. Ссылка не
    /// освобождается никогда: подложка живёт весь процесс, `into_raw`
    /// отпускает её из-под учёта ARC намеренно, а не по забывчивости.
    ...
    fn ensure_window(mtm: MainThreadMarker) -> Result<*mut NSWindow, String> {
        ...
        // Главный экран, а не монитор пикера: назвать монитор пикера здесь
        // без ручного построения `NSRect`/`NSPoint` нечем — а строить их
        // пришлось бы через `objc2-foundation`, отдельный крейт сверх двух
        // разрешённых для этой задачи (`objc2`, `objc2-app-kit`). Подложка на
        // вторичном мониторе останется на главном — известное отступление,
        // см. отчёт о задаче.
    ...
    pub fn set_visible(_app: &tauri::AppHandle, show: bool) -> Result<(), String> {
        // AppKit целиком главнопоточный; команда, пришедшая не оттуда,
        // честнее откатить с ошибкой, чем разбираться с окном не с того
        // потока.
        let mtm = MainThreadMarker::new()
            .ok_or_else(|| "scrim must be toggled from the main thread".to_string())?;
