...
/// windowHost при этом не пуст, — и со своей же машины, где Enter вызывает эту
/// же команду напрямую, когда `chooseOpenTransport` отдал `manager`. Транспорт
/// один и тот же в обоих случаях: HTTP здесь не сработал бы даже на своей
/// машине — webview Tauri режет такой запрос как cross-origin ещё до отправки.
/// Ответа у просьбы нет по той же причине, что и у фокуса: приёмник
/// отчитывается в свой лог, а не нам.
#[tauri::command]
