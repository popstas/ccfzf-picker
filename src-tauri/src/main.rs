...
/// между долей и пикселями, `(100..101)`, и всё, что меньше нуля или не
/// число, читаются как мусор и откатываются на встроенный размер — со
/// строкой в stderr: правку руками надо либо исполнить, либо объяснить, а
/// молчаливый откат выглядит потерянной настройкой.
fn scale_axis(node: Option<&serde_json::Value>, name: &str) -> f64 {
    ...
    eprintln!(
        "ccfzf-picker: pickerSize.{name} = {pct} is outside 1..100 and below 101, using the built-in size"
    );
    0.0
}
