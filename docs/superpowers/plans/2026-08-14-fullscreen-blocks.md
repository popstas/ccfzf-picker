...
- [ ] `^F` разворачивает окно до 1400×900 по центру (на экране поменьше — до зажатого по нему размера, см. пункт про маленький экран ниже), повторный `^F` возвращает 900×640 по центру.
...
Дописано по итогам починок, сделанных уже после того, как план написали, — это
ловится только глазами:

- [ ] Карточка не протекла на соседей: в широком режиме строка проекта,
  заголовок снимка, сессия снимка, строка `N sessions · last …` и **зелийная**
  строка остаются однострочными, метаданные справа на той же линии, а не под
  текстом.
- [ ] Кружок стоит у первой строки имени карточки, а не над ним — проверять на
  сессии с длинным именем, которое переносится на две строки.
- [ ] Запрос и ответ в карточке правда занимают до трёх строк и обрезаются по
  последней: на WebView2 это правило никто не видел, измеряли в постороннем
  Chromium.
- [ ] Блоков больше, чем колонок (два ряда и больше; легко получить, включив
  `showAll`): все блоки видны и достижимы, ни один не срезан нижним краем,
  общий скролл страницы не появляется.
- [ ] Круг через окно настроек: в широком режиме открыть настройки, тронуть
  галку на вкладке UI, сохранить — пикер обязан остаться широким и рамой, и
  раскладкой.
- [ ] Широкий режим на самом маленьком из используемых экранов: видна ли строка
  статуса внизу, не уехало ли окно под строку меню, остаётся ли видно
  что-нибудь под окном (окно должно зажиматься по экрану — `fit_to_screen`).
  Проверить обоими путями к одному размеру: и нажатием `^F` на открытом
  пикере, и перезапуском пикера с запомненным широким режимом — до второго
  фикса они вели себя по-разному, потому что при перезапуске окно ещё скрыто
  в момент первой просьбы о размере.
- [ ] Клик по строке в третьем блоке выбирает именно её (порядок `rows` против
  порядка DOM), и `←/→` сразу после показа окна с непустым запросом двигают
  каретку, а не блоки: на показе `search.select()` выделяет текст, а при
  выделении стрелка в навигацию не уходит — поведение верное, но выглядит как
  «стрелки не работают».
