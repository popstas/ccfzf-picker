...
- [ ] Выкатить на все три машины (`BRANCH=<ветка> ./data/scripts/deploy-win.sh` и `BRANCH=<ветка> ./data/scripts/deploy-mac.sh --all`, параллельно и в фоне), трекеры — своими путями. Порядок не важен.
- [ ] Скрыть пикер, подождать больше минуты, открыть терминал из пикера (`^N`). Открыть пикер: строка обязана быть с пометкой ▣ **сразу**, а не появиться через секунду после открытия.
- [ ] Скрыть пикер, подождать больше минуты, закрыть окно сессии руками. Открыть пикер: пометки ▣ у строки быть не должно.
- [ ] Скрыть пикер, нажать проектный хоткей каталога, у которого сессия уже открыта на этой машине. Второй сессии с тем же именем появиться не должно.
- [ ] Окно настроек, вкладка Window: `Tracker signal` показывает секунды, а не `never seen`. Показывает `never seen` — разошёлся путь между репозиториями, смотреть Global Constraints.
- [ ] Проверить, что alt-tab между терминалами **не** гонит опрос: при скрытом пикере переключение окон не должно вызывать обращений к ssh.
