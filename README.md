...
- **Проекты** — справа: звёздочка избранного, число сессий и живых среди них
  ...
  проекта мимо пикера) и давность последней работы.
- **Строка сессии** — имя, проект, последний запрос человека и ответ агента.
  Устаревшие приглушены.
...
- **Панели сворачиваются и переставляются** — `Snapshots - 20` и
  ...
  колонками, порядок переживает перезапуск.
- **Статуслайн** — сортировка, подсказка меню `^K`, галки-фильтры (`project`,
  `only windowed`, `show all`), справочник клавиш, переключатель широкого
  режима и шестерёнка настроек.
...
- **Window size** — по ширине и высоте на каждую раскладку, узкую и широкую:
