...
- **Window** — размер окна: по выбору ширины и высоты на каждую раскладку,
  узкую и широкую. `Default` — встроенный размер, одинаковый на любом экране;
  остальные пункты — доля экрана. На большом мониторе `80% of screen` по высоте
  узкого списка вмещает заметно больше сессий.
