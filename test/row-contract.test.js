...
test('снимок отделён от соседа сильнее, чем его сессии друг от друга', () => {
  // Группы читаются отступами, а не разделителями, и порядок величин тут и
  // есть правило: между снимками больше базовых 7px, внутри снимка меньше.
  // Переставь их местами — снимок слипся бы с соседом, а его сессии разъехались
  // бы, и на экране пропала бы сама граница группы. Поймать это можно только
  // глазами или вот так, по тексту: разметка в обоих случаях одна и та же.
  const px = (re) => {
    const found = SESSIONS_HTML.match(re);
    assert.ok(found, `правило не найдено: ${re}`);
    return Number(found[1]);
  };
  const base = px(/\.row \{[^}]*padding: (\d+)px \d+px;/);
  const between = px(/\.row\.snapshot \{ padding-top: (\d+)px; \}/);
  const inside = px(/\.row\.snapshot-session \{ padding-top: (\d+)px;/);
  assert.ok(between > base, `между снимками (${between}) должно быть больше базового (${base})`);
  assert.ok(inside < base, `внутри снимка (${inside}) должно быть меньше базового (${base})`);
});
...
test('старые обычные сессии и проекты получают stale, а Zellij нет', () => {
  const stale = { enabled: true, sessionHours: 2, projectHours: 24, opacity: 0.4 };
