...
test('Ctrl+M занята комментарием и настроенному действию не достаётся', () => {
  // Правило CLAUDE.md, за которое уже заплачено дважды — списком `c v x z` и
  // `KeyF`: клавиша, которую слушает встроенная ветка, обязана стоять в
  // RESERVED_CODES, иначе настроенное человеком действие на ней молчит.
  //
  // Один список на всё: буква приходит из BUILTIN_ACTION_KEYS, и подписью в
  // меню, и хоткеем, и запретом — переписанный руками второй разошёлся бы с
  // первым молча.
