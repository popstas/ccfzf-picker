...
// 'other' добавлено при первом прогоне: test/session-glyph.test.js:194
// намеренно использует /home/other/x рядом с /home/user/... (см. комментарий
// в том же файле на строке 193), чтобы показать, что имя пользователя нигде
// не зашито жёстко.
const SAMPLE_USERS = new Set(['user', 'example', 'someone', '<user>', 'other']);
...
// имён не требуют.
const UNIVERSAL = [
  { re: /\/home\/([A-Za-z][\w.-]*)\//g, what: 'домашний каталог Linux с именем пользователя' },
  { re: /\/Users\/([A-Za-z][\w.-]*)\//g, what: 'домашний каталог macOS с именем пользователя' },
  { re: /\\Users\\([A-Za-z][\w.-]*)\\/g, what: 'домашний каталог Windows с именем пользователя' },
];
