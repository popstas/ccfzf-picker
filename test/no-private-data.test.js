...
// шире одних лишь имён машин и домашнего пути. В пикселях там видны: путь
// `/home/popstas/...`; два имени машин подзаголовками чужих сессий (`mac`,
// `macbook`); полный список проектов текстом (`ExpertizeMe`,
// `planfix-mcp-server`, `ansible-server-popstas`, `smi-parser`,
// `cup-dashboard`, `telegram-functions-bot` и другие); и обычным текстом —
// содержимое сессий: упоминания созвонов и Планфикса, хэш коммита со списком
// изменённых файлов, заметки про вольт. Всё это — ровно те данные, которые
// остальным файлам этот же страж запрещает. Ни прочитать, ни отредактировать
// их проверке нечем; решение опубликовать снимок принято владельцем
// сознательно, запись здесь лишь называет цену вслух — целиком, а не только
// её часть. Второму снимку тут появляться не следует, не замазав на нём эти
// же классы данных.
