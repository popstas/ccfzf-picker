...
// Отслеживаемые файлы, которые страж читать не может. Восемь из девяти —
// иконки приложения: семь из них вывод `python3 scripts/make-icons.py`,
// восьмая (`favicon.png`) правится руками, и имени машины в пикселях там не
// бывает. Список задан именами, а не маской вроде «всё двоичное»: маска молча
...
//
// Девятый — снимок экрана для README, и он **исключение с известной ценой**,
// шире одних лишь имён машин. В пикселях там видны, классами: домашний путь
// владельца целиком; имена машин подзаголовками чужих сессий; весь список
// проектов, включая непубличные; и содержимое сессий обычным текстом —
// переписка с агентом, хэш коммита со списком изменённых файлов, упоминания
// внутренних систем. То есть ровно те данные, которые остальным файлам этот
// же страж запрещает. Ни прочитать, ни отредактировать их проверке нечем;
// решение опубликовать снимок принято владельцем сознательно, запись здесь
// лишь называет цену вслух — целиком, а не только её часть.
//
// Перечислено классами, а не значениями, и это не стыдливость: файл стоит в
// `ALLOWED`, то есть сам себя страж не читает, и выписанное сюда настоящее
// имя машины или домашний путь уехали бы в публикацию тем же молчанием, каким
// уезжают пиксели. Первая редакция этого комментария именно так и сделала.
// Второму снимку тут появляться не следует, не замазав на нём эти же классы.
...
test('список отслеживаемых файлов содержит только файлы, не директории', () => {
  // Содержимое чужого репозитория (`vendor/ccfzf`) — не файл этого: он
  // отвечает за свои приватные данные сам, а gitlink на него в списке — уже
  // регрессия фильтра выше. Директорией такая запись и опознаётся: как файл
  // её не прочитать вовсе, `fs.readFileSync` упадёт EISDIR — и упавший этим
  // тест погасил бы сразу все три стража, ничего не проверив. Отсюда и место
  // проверки — раньше любой попытки textLines().
