...
test('каждая подложка оверлея спрятана по умолчанию и показывается классом open', () => {
  // Отказ здесь тихий и полный: подложка без `display: none` стоит поверх
  // списка всегда — пикер выглядит открывшимся в пустой серый прямоугольник, и
  // ни в одной ветке кода этого не видно. Ровно так и приехала подложка
  // комментария, дописанная разметкой раньше, чем стилем.
  const styles = SESSIONS_HTML.split('</style>')[0];
  const ids = [...SESSIONS_HTML.matchAll(/<div id="([a-z-]+-backdrop)"/g)].map(m => m[1]);
  assert.ok(ids.length >= 3, `подложек нашлось ${ids.length} — сломалась сама выборка`);
  for (const id of ids) {
    assert.ok(new RegExp(`#${id}[^{]*\\{[^}]*display:\\s*none`).test(styles),
      `подложка #${id} не спрятана по умолчанию — она закроет список навсегда`);
    assert.ok(new RegExp(`#${id}\\.open`).test(styles),
      `подложку #${id} нечем показать: правила .open нет`);
  }
});
