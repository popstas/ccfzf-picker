...
test('наведение на старую строку меняет только прозрачность, не фон', () => {
  // У наведённой stale-строки срабатывали оба сигнала сразу: прозрачность
  // 0.5 → 1 и фон. Скачок прозрачности — самая заметная перемена в списке, и
  // фон поверх него делал строку под мышью такой же громкой, как выделенная.
  //
  // `:not(.active)` обязателен. Без него вес правила (0,3,0) побил бы
  // `.row.active` (0,2,0), и выделенная старая строка под мышью теряла бы фон
  // выделения — то есть спор с выделением решался бы в пользу наведения,
  // ровно наоборот тому, что удерживает порядок правил у `.row:hover`.
  assert.ok(/\.row\.stale:not\(\.active\):hover\s*\{[^}]*background:\s*none/.test(STYLES),
    'наведение на старую строку снова красит фон');
  // Обратная сторона той же правки: сузить сам `.row:hover` до
  // `:not(.stale)` нельзя — это подняло бы вес наведения над `.row.active`, и
  // наведение стало бы гасить выделение уже у обычных строк.
  assert.ok(/(^|[^)])\.row:hover\s*\{/m.test(STYLES),
    'общее правило наведения сузили — оно обязано остаться на всех строках');
});
